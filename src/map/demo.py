...
def getImages(i):
    return cv2.imread('/home/hoaiphuong/phuong/2d/image/{0:03d}.jpg'.format(i),0)

def getK():
    return   np.array([[1900, 0, 910],
              [0, 1900, 380],
              [0, 0, 1]])
...
cap = cv2.VideoCapture('IMG_0448.MOV')
f, img = cap.read()
img_1 = img
f, img = cap.read()
img_2 = img
...
#Camera parameters
fc = 1930.081569625214
pp = (932.9878308679828, 401.11453072300174)
K  = getK()
...
#    if absolute_scale > 0.1:
    ...
    draw_x, draw_y = int(t_f[0]) + 500, int(t_f[2]) + 500;
...
#    cv2.circle(traj, (draw_tx, draw_ty) ,1, (255,0,0), 2);
    ...
    cv2.putText(traj, text, (10,50), cv2.FONT_HERSHEY_PLAIN, 1, (255,255,255), 1, 8);
    img2 = cv2.drawKeypoints(curImage_c, kp1, None,color=(255,0,0))
